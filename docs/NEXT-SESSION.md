...
| Load-test rig | **down**, host sysctls restored (`rmem_max` 33554432). Host cleaned 2026-09-04 at the user's request: the rig images (`snuffles-sut/gen/sink`) and the Wine test image are deleted (`rig.sh build` and `scripts/test-windows.sh` rebuild them, a few minutes), `pktgen` unloaded, project scratch under `/tmp/claude-1000/-home-ixo-snuffles` removed. The `snuffles-win` KVM guest and its storage pool are gone too: real-Windows verification needs a new VM |
